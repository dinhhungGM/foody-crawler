...
class FoodyCommentCrawler:
    ...
    def create_driver(self, headless: bool = False, debug_mode: bool = False, fast_load: bool = True) -> webdriver.Chrome:
        ...
        if driver is None:
            ...
            driver = webdriver.Chrome(executable_path=driver_path, options=options,
                                      desired_capabilities=capa, service_log_path=os.devnull)
    ...
    def export_food_shops(self, food_shops: List[dict], location: str) -> None:
        FOODSHOP_CSV_PATH = f'results/foodshops_{location}.csv'
        foodshop_df = pd.DataFrame.from_dict(food_shops)
        foodshop_df.to_csv(FOODSHOP_CSV_PATH, index=False)

    def read_food_shops(self, shop_name: str) -> List[FoodShop]:
        foodshop_df = pd.read_csv(f'results/foodshops_{shop_name}.csv')
        food_shops: List[dict] = foodshop_df.to_dict(orient='records')
        food_shops = [FoodShop(**food_shop) for food_shop in food_shops]
        return food_shops

    def login(self, driver: webdriver.Chrome, location: str):
        driver.get(
            f'https://id.foody.vn/account/login?returnUrl=https://www.foody.vn/{location}')
    ...
    def get_content_item_elements(self, driver: webdriver.Chrome, location: str) -> List[WebElement]:
        n_items: int = 0
        ...
        food_shops: List[dict] = self.get_food_shops(
            driver=driver, content_item_eles=content_item_eles)
        self.export_food_shops(food_shops, location)

        while 1:
            try:
                xemthem_btn_ele = self.wait_find(
                    driver=driver, selector_str='#ajaxRequestDiv > div > div.content-container.fd-clearbox.ng-scope > div.pn-loadmore.fd-clearbox.ng-scope > a > label', selector_type='css', num_ele='one')
                content_item_eles: List[WebElement] = []
                if xemthem_btn_ele:
                ...
                if len(content_item_eles) > 0:
                    food_shops.extend(self.get_food_shops(
    ...
    def get_food_shops(self, driver, content_item_eles: List[WebElement]) -> List[dict]:
        def process(content_item_ele: WebElement) -> FoodShop:
            title_ele: WebElement = content_item_ele.find_element_by_class_name(
                'title')
            ...
            return food_shop
        food_shops: List[FoodShop] = CommonUtils.process_list(
            inputs=content_item_eles, func=process, desc='Getting food shops', method='multi')
        food_shops = [shop.to_dict() for shop in food_shops]
        return food_shops

    def go_get_food_shops(self, location: str):
        driver = self.create_driver()
        driver.get(self.HOMEPAGE_URL)
        # login
        self.login(driver, location)
        # get FoodShop information
        content_item_eles: List[WebElement] = self.get_content_item_elements(
            driver, location)
        driver.quit()

    def go_get_food_shops_from_locations(self, locations: List[str]):
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            try:
                executor.map(crawler.go_get_food_shops, locations)
            except Exception as e:
                print(e)
                pass
    ...
    def get_comments_from_one_food_shop(self, food_shop: FoodShop) -> List[Comment]:
        ...
        # get shop info
        shop_rating_ele: WebElement = self.wait_find(
            driver=driver, selector_str='#res-summary-point > div > div.microsite-top-points-block > div.microsite-point-avg')
        shop_rating = shop_rating_ele.text if shop_rating_ele else ''
        shop_n_comments_eles: WebElement = self.wait_find(
            driver=driver, selector_str='summary', selector_type='class', num_ele='many')
        shop_n_comments = '-1'
        ...
        review_item_eles: List[WebElement] = []
        while 1:
            review_item_eles: List[WebElement] = self.wait_find(
                driver=driver, selector_str='review-item', selector_type='class', num_ele='many')
            self.scroll_to_bottom(driver)
            sleep(self.SCROLL_PAUSE_TIME)
        ...
        while 1:
            try:
                xemthem_btn_eles: List[WebElement] = self.wait_find(
                    driver=driver, selector_str='fd-btn-more', selector_type='class', num_ele='many')
                ...
                if xemthem_btn_ele:
                    ...
                    #     comments: List[Comment] = self.get_comment_info(review_item_eles=review_item_eles, shop_name=food_shop.name, shop_url=food_shop.url, shop_rating=shop_rating, shop_n_comments=shop_n_comments)
                    logger.info(
                        f'{len(review_item_eles)} / {int(shop_n_comments)} ~ {(len(review_item_eles)*100/int(shop_n_comments)):.2f} % items loaded')
                else:
                    break
                n_tries = 1
            except Exception as e:
                ...
                n_tries += 1
                continue
        comments: List[Comment] = self.get_comment_info(
            review_item_eles=review_item_eles, shop_name=food_shop.name, shop_url=food_shop.url, shop_rating=shop_rating, shop_n_comments=shop_n_comments)
        driver.quit()
        return comments
    ...
    def get_comment_info(self, review_item_eles: WebElement, shop_name: str, shop_url: str, shop_rating: str, shop_n_comments: str) -> List[Comment]:
        def process(review_item_ele: WebElement) -> Comment:
            comment_info = None
            try:
                commentor_ele: WebElement = self.wait_find(
                    driver=review_item_ele, selector_str='ru-username', selector_type='class')
                commentor: str = commentor_ele.text if commentor_ele else ''
                content_ele: WebElement = self.wait_find(
                    driver=review_item_ele, selector_str='rd-des', selector_type='class')
                content_ele: WebElement = self.wait_find(
                    driver=content_ele, selector_str='span', selector_type='tag')
                content: str = content_ele.text if content_ele else ''
                rating_ele: WebElement = self.wait_find(
                    driver=review_item_ele, selector_str='review-points', selector_type='class')
                rating: str = rating_ele.text if rating_ele else ''
                updated_at: str = CommonUtils.get_date_time()
                comment_info = Comment(commentor=commentor, content=content, rating=rating, updated_at=updated_at,
                                       shop_name=shop_name, shop_url=shop_url, shop_rating=shop_rating, shop_n_comments=shop_n_comments)
            except Exception as e:
                logger.error(str(e))
            return comment_info
        comments: List[Comment] = CommonUtils.process_list(
            inputs=review_item_eles, func=process, desc='Getting comment info', method='single')
        return comments

    def go_get_comments(self, shop_name: str):
        driver = self.create_driver()
        driver.get(self.HOMEPAGE_URL)
        # login
        self.login(driver, shop_name)
        food_shops: List[FoodShop] = self.read_food_shops(shop_name)

        def process_and_export(food_shops: List[FoodShop], start: int, end: int):
            all_comments: List[dict] = []
            results: List[List[Comment]] = CommonUtils.process_list(
                inputs=food_shops[start:end], func=self.get_comments_from_one_food_shop, desc='Getting comments of all shops - Progress', method='multi')
            for r in results:
                for e in r:
                    if e:
            ...
            df.to_csv(
                f'results/comments_{start}_{end}_{shop_name}.csv', index=False)
            logger.info(
                f'Exported to results/comments_{start}_{end}_{shop_name}.csv')
        ...
        for i in range(start, end, step):
            try:
                process_and_export(food_shops=food_shops,
    ...
    def test(self):
        ...
        def process(food_shop: FoodShop) -> dict:
            ...
            url = 'https://www.foody.vn/' + \
                url[index+len('https://www.foody.vn/https://www.foody.vn/'):]
            return FoodShop(name=food_shop.name, url=url, updated_at=food_shop.updated_at).to_dict()
        results: List[dict] = CommonUtils.process_list(
            inputs=food_shops, func=process, desc='Getting food shops', method='single')
    ...
    def merge_foodshop_files(self):
        df = pd.DataFrame()
        for f in glob('results/foodshops_*.csv'):
            df = df.append(pd.read_csv(f))
        df.to_csv('results/foodshops.csv', index=False)

        # Remove all foodshops_location
        # for file in glob('results/foodshops_*.csv'):
        #     os.remove(file)


if __name__ == '__main__':
    locations = ['ho-chi-minh', 'ha-noi', 'da-nang', 'can-tho', 'khanh-hoa', 'vung-tau', 'hai-phong', 'binh-thuan', 'lam-dong', 'dong-nai', 'quang-ninh', 'hue', 'binh-duong', 'hai-duong', 'ninh-thuan', 'nam-dinh', 'tien-giang', 'phu-quoc', 'kon-tum', 'quang-nam', 'lao-cai', 'nghe-an', 'long-an', 'dien-bien', 'binh-dinh', 'dak-lak', 'thanh-hoa', 'phu-yen', 'bac-ninh', 'thai-nguyen', 'an-giang',
                 'kien-giang', 'quang-ngai', 'tay-ninh', 'gia-lai', 'binh-phuoc', 'vinh-long', 'ca-mau', 'dong-thap', 'quang-tri', 'quang-binh', 'hoa-binh', 'vinh-phuc', 'ben-tre', 'thai-binh', 'soc-trang', 'ninh-binh', 'tra-vinh', 'bac-giang', 'hung-yen', 'bac-lieu', 'ha-tinh', 'phu-tho', 'hau-giang', 'son-la', 'lang-son', 'dak-nong', 'ha-nam', 'ha-giang', 'tuyen-quang', 'yen-bai', 'cao-bang', 'bac-kan', 'lai-chau']
    crawler = FoodyCommentCrawler()
    # crawler.go_get_food_shops_from_locations(locations)

    for location in locations:
        crawler.go_get_comments(shop_name=location)

    crawler.merge_foodshop_files()
